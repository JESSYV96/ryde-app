...
export class StripePaymentGateway implements PaymentGateway {
  ...
  async createCheckoutSession(request: PaymentLinkRequest): Promise<CheckoutSession> {
    ...
    const session = await this.stripe.checkout.sessions.create({
      ...
      // Carried through Stripe metadata so the stateless payment.completed event
      // can drive both the company push and the customer confirmation email
      // without any server-side storage.
      metadata: buildMetadata({
        rentalId: request.rentalId,
        customerEmail: request.customerEmail,
        customerName: request.customerName,
        amount: String(request.money.amount),
